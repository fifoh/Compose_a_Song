...
function draw() {
  ...
  for (let i = 0; i < ellipses.length; i++) {
    ...
    pointSize = windowWidth * 0.3 / numEllipses; // was 0.3
    for (let j = ellipseData.points.length - 1; j >= 0; j--) {
      let band_point = ellipseData.points[j];
      let { angle } = band_point;
      let pointX = ellipseData.centerX + ellipseWidth / 2 * Math.cos(angle);
      let pointY = centerY + ellipseHeight / 2 * Math.sin(angle);
      let verticalSize = map(1 - pow(abs(Math.sin(angle)), 10), 0, 1, 1, 10); // last number was 10
...
function touchEnded() {
  ...
  for (let i = 0; i < ellipses.length; i++) {
    ...
    for (let j = ellipseData.points.length - 1; j >= 0; j--) {
      ...
      if (dist(touchX, touchY, pointX, pointY) <= pointSize / 1) { // was "/ 2" to delete point
